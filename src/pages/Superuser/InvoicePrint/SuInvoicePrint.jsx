...
import { SuComponentToPrint } from "./SuComponentToprint";
...
const SuInvoicePrint = (props) => {
  ...
  return (
    <div className="invPrint">
          ...
          >
            {({ zoomIn, zoomOut, setTransform, ...rest }) => {
              return (
                ...
                >
                  <SuComponentToPrint ref={componentRef} data={props.propsData}  />
                </TransformComponent>
              );
            }}
          </TransformWrapper>
     ...
     {
     props?.onClose&&<button className="PdfButton" onClick={()=>props?.onClose?props?.onClose():""}>Close</button>
     }
      </div>
    </div>
  );
};
export default SuInvoicePrint