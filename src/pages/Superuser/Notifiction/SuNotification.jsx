...
// import { error, success } from '../../t'
import { toast } from 'react-toastify'
import { Refresh } from '@mui/icons-material'

function SuNotification(props) {
    ...
    }, [inNotification, incompany,Refresh])
    useEffect(() => {
                                            ...
                                            AdminNoti?.map(item => {
                                                return (
                                                    <div>
                                                        <div onClick={() => handelExpand(item.id)} className='BidNoti' >
                                                            <li  >{`${item?.message.slice(0, 30)}...`}</li>
                                                            <span onClick={() => deletNotification(item.id)}><ClearIcon /></span>
                                                        </div>
                                                        {
                                                            expand == item?.id && (
                                                                <div className='showNoti' >{item?.message}</div>
